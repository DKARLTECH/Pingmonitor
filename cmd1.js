...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    // Store custom channels for this user
    customChannelsMap.set(chat_id, channels);
    
    // List the custom channels for the user with pagination (starting at page 0)
...
async function processUploadedM3UFile(message) {
  const chat_id = message.chat.id;
  try {
    // For simplicity, assume that the file URL is provided in message.document.url
    // In a real scenario, you'd need to use Telegram's getFile API to retrieve the file path.
    const fileUrl = message.document.url;
    ...
    const content = await response.text();
    console.log("Fetched Uploaded File Content:", content);
    ...
    // Store custom channels for this user from the uploaded file
    customChannelsMap.set(chat_id, channels);
    await sendMessage(chat_id, "Custom channels from the uploaded file have been processed successfully.");
    // Optionally, list the custom channels for the user
    await listCustomChannels(chat_id, 0);
  } catch (error) {
...
async function listCustomChannels(chat_id, page, message_id = null) {
  ...
  if (channels.length === 0) {
    if (message_id) {
      await editMessage(chat_id, message_id, "No custom channels found. Please add a custom URL or upload a file.");
    } else {
      await sendMessage(chat_id, "No custom channels found. Please add a custom URL or upload a file.");
    }
    return;
  }
