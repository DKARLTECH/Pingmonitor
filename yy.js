...
async function processTelegramMessage(message) {
  ...
  if (message.text === "/admin") {
    if (chat_id === ADMIN_CHAT_ID) {
      ...
      const adminMessage = "╔═🌟 Admin Dashboard ☆═╗\nCurrent M3U URL:\n`" + M3U_URL + "`\n\nClick the buttons below to update the URL or send a broadcast message.";
...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    // Updated share button: provide a link to join the telegram group/chat as per new requirements
    await sendMessage(chat_id, "Join our free IPTV stream group: https://t.me/Freeiptvstream_bot");
...
async function processUploadedM3UFile(message) {
  ...
  try {
    ...
    const getFileResponse = await fetch(`${TELEGRAM_API}/getFile?file_id=${message.document.file_id}`);
    const getFileData = await getFileResponse.json();
    if (!getFileData.ok) {
      console.error("Error getting file:", getFileData);
      await sendMessage(chat_id, "Error retrieving the file. Please try again.");
      return;
    }
    ...
    if (!fileResponse.ok) {
      console.error(`Failed to fetch the .m3u file, status: ${fileResponse.status}`);
    ...
    await sendMessage(chat_id, "Error processing uploaded file. Please try again.");
  }
}

// List custom channels for a user with pagination
async function listCustomChannels(chat_id, page, message_id = null) {
  const userData = customChannelsMap.get(chat_id);
  if (!userData || !userData.channels || userData.channels.length === 0) {
    await sendMessage(chat_id, "No custom channels found. Please add one by sending a valid custom URL or uploading a .m3u file.");
    return;
  }
...
async function playCustomChannel(chat_id, id) {
  ...
  if (id >= 0 && id < channels.length) {
    ...
    const channelUrl = bypassGeo(channel.url);
    let message = `Now Playing Custom Channel: ${channel.name}\n\n` +
