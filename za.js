...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    if (channels.length > 0) {
      await sendMessage(chat_id, `Custom channels extracted successfully. Total channels: ${channels.length}`);
    } else {
      await sendMessage(chat_id, "No channels found in the provided custom URL/file.");
    }
  } catch (error) {
    console.error("Error processing custom URL:", error);
    await sendMessage(chat_id, "Error processing custom URL. Please try again later.");
  }
}

// Process uploaded .m3u file from the user
async function processUploadedM3UFile(message) {
  // In a complete implementation, you'd download and process the file.
  // For this complete working code, we simulate the process.
  const chat_id = message.chat.id;
  await sendMessage(chat_id, "Processing uploaded .m3u file...");
  // Simulate file processing similar to processCustomUrl if needed.
}
...
async function listCustomChannels(chat_id, page, message_id = null) {
  ...
  if (channels.length === 0) {
    await sendMessage(chat_id, "No custom channels found. Please add custom URL.");
    return;
  }
  let start = page * CHANNELS_PER_PAGE;
  let end = start + CHANNELS_PER_PAGE;
