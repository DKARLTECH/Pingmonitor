...
// Removed the time limiting rate for requests per chat to make it faster
...
// Rate limiting removed; the structure is retained for possible future use
const rateLimitMap = new Map();

// In-memory custom channels store per user for custom URL/file submissions and uploaded M3U files
...
// Helper function to check rate limiting for a chat (disabled for faster response)
function isRateLimited(chat_id) {
  // Rate limiting has been disabled; always return false.
  return false;
  /*
  const now = Date.now();
...
async function processTelegramMessage(message) {
  ...
  if (message.document && message.document.file_name && 
      (message.document.file_name.endsWith(".m3u") || message.document.file_name.endsWith(".m3u8") || message.document.file_name.endsWith(".ts"))) {
    await processUploadM3U(chat_id, message);
    return;
  }
  
  // Check for rate limit to avoid congestions/spams (disabled)
...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    // Prompt the user to upload their M3U file via Telegram document upload
    await sendMessage(chat_id, "Please upload your M3U file as a document (supports .m3u, .m3u8, .ts):");
...
async function processUploadM3U(chat_id, message) {
  try {
    const file_id = message.document.file_id;
    // Get file information from Telegram
    const fileInfoResponse = await fetch(`${TELEGRAM_API}/getFile?file_id=${file_id}`, { method: "GET" });
    const fileInfoData = await fileInfoResponse.json();
    if (!fileInfoData.ok) {
      console.error("Failed to get file info:", fileInfoData);
      await sendMessage(chat_id, "Failed to retrieve the file information. Please try again.");
      return;
    }
    const file_path = fileInfoData.result.file_path;
    ...
    if (channels.length === 0) {
      console.log("No channels found in the uploaded M3U file.");
