  ...
  if (text === "/start") {
    let keyboard = {
      inline_keyboard: [
        ...
        [{ text: "🤝 Share Bot", url: "https://t.me/Freeiptvstream_bot" }]
      ],
    };
...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    if (subscribers.has(chat_id)) {
      ...
      await sendMessage(chat_id, "You have been unsubscribed from notifications.");
    } else {
      await sendMessage(chat_id, "You are not currently subscribed.");
    }
...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    if (channels.length === 0) {
      ...
      // Save custom channels for the user
      customChannelsMap.set(chat_id, channels);
      await sendMessage(chat_id, `Custom channels added successfully. Found ${channels.length} channels.`);
    }
  } catch (error) {
...
// Process an uploaded .m3u file from Telegram
async function processUploadedM3UFile(message) {
  ...
  try {
    ...
    if (!fileInfoData.ok) {
      await sendMessage(chat_id, "Failed to retrieve file information.");
      return;
    }
    ...
    if (channels.length === 0) {
      ...
      // Save custom channels from the uploaded file for the user
      customChannelsMap.set(chat_id, channels);
    ...
    await sendMessage(chat_id, "Error processing the uploaded file.");
  }
}
...
// Play a custom channel chosen from the custom channels list
async function playCustomChannel(chat_id, id) {
  const channels = customChannelsMap.get(chat_id);
  if (!channels || id < 0 || id >= channels.length) {
  ...
  let channelUrl = bypassGeo(channel.url);
  let message = `Now Playing: ${channel.name}\n\n` +
      `📺 To watch this channel, copy the URL below and paste it into your media player's network stream option:\n` +
      `\`${channelUrl}\`\n\n` +
      `Instructions:\n` +
      `- For VLC: Open VLC, go to Media > Open Network Stream, then paste the URL.\n` +
      `- For MX Player: Open MX Player, tap the menu and select Network Stream, then paste the URL.`;
