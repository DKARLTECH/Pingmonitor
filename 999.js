...
// Rate limiting has been disabled to handle requests faster
function isRateLimited(chat_id) {
  return false;
  /*
  // Original rate limiting implementation preserved for reference:
  const now = Date.now();
...
async function processTelegramMessage(message) {
  ...
  // Check for rate limit to avoid congestions/spams (now disabled)
  ...
  if (text === "/start") {
    let keyboard = {
      inline_keyboard: [
        ...
        [{ text: "B. Add m3u Custom URL", callback_data: "custom_url_b" }],
...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    await sendMessage(chat_id, "Please send your custom URL (only m3u or m3u8_plus links are allowed):");
  } else if (data === "custom_url_b") {
    // Additional menu option B for custom URL submission,
    // activated when option A fails or the user selects it.
    pendingCustomUrlMap.set(chat_id, true);
    await sendMessage(chat_id, "Please send your custom URL (only m3u or m3u8_plus links are allowed):");
