...
async function processUploadedM3UFile(message) {
  ...
  try {
    ...
    const getFileResponse = await fetch(`${TELEGRAM_API}/getFile?file_id=${fileId}`);
    const getFileData = await getFileResponse.json();
    if (!getFileData.ok) {
      console.error("Error getting file info:", getFileData);
      await sendMessage(chat_id, "Error retrieving file information.");
      return;
    }
    ...
    if (!response.ok) {
      console.error(`Failed to fetch the uploaded file, status: ${response.status}`);
    ...
    console.log("Extracted Custom Channels from file:", channels);
...
async function listCustomChannels(chat_id, page, message_id = null) {
  const customData = customChannelsMap.get(chat_id);
  if (!customData || !customData.channels || customData.channels.length === 0) {
    await sendMessage(chat_id, "No custom channels found. Please submit a custom URL or upload a .m3u file.");
    return;
  }
  
  const channels = customData.channels;
  let start = page * CHANNELS_PER_PAGE;
  let end = start + CHANNELS_PER_PAGE;
  let paginatedChannels = channels.slice(start, end);
...
async function playCustomChannel(chat_id, id) {
  const customData = customChannelsMap.get(chat_id);
  if (!customData || !customData.channels) {
