...
async function listCustomChannels(chat_id, page, message_id = null) {
  ...
  // Arrange custom channel buttons vertically
  let channelButtons = paginatedChannels.map((channel, index) => {
...
// Play a custom channel inside Telegram from the user's custom channels
async function playCustomChannel(chat_id, id) {
  ...
  if (id >= 0 && id < channels.length) {
    ...
    let channelUrl = bypassGeo(channel.url);
    let message = `Now Playing: ${channel.name}\n\n` +
      `📺 To watch this channel, copy the URL below:\n` +
      `\`${channelUrl}\``;
    await sendMessage(chat_id, message);
  } else {
...
async function processUploadedM3UFile(message) {
  ...
  try {
    ...
    const getFileResponse = await fetch(`${TELEGRAM_API}/getFile?file_id=${file_id}`);
    const fileData = await getFileResponse.json();
    if (!fileData.ok) {
      await sendMessage(chat_id, "Failed to retrieve the file.");
      return;
    }
