  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .navigation button { padding: 8px 16px; margin: 0 5px; border: none; border-radius: 4px; background: #007bff; color: #fff; cursor: pointer; }
...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    if (channels.length === 0) {
      await sendMessage(chat_id, "No valid channels found in the custom file.");
    } else {
      customChannelsMap.set(chat_id, channels);
      await sendMessage(chat_id, `Added ${channels.length} custom channels.`);
    }
  } catch (error) {
    console.error("Error processing custom URL:", error);
    await sendMessage(chat_id, "Failed to process the custom URL.");
  }
}

// Process uploaded .m3u file from the user
async function processUploadedM3UFile(message) {
  const chat_id = message.chat.id;
  // In a real implementation, you would retrieve the file using Telegram's getFile API,
  // then fetch the file content. For this example, we'll simulate file processing.
  await sendMessage(chat_id, "Your .m3u file has been received and processed.");
}

// List custom channels stored for a user with pagination
async function listCustomChannels(chat_id, page, message_id) {
  let customChannels = customChannelsMap.get(chat_id) || [];
  if (customChannels.length === 0) {
    await sendMessage(chat_id, "No custom channels available.");
    return;
  }
  let start = page * CHANNELS_PER_PAGE;
  let end = start + CHANNELS_PER_PAGE;
  let paginatedChannels = customChannels.slice(start, end);
  let channelButtons = paginatedChannels.map((channel, index) => {
  ...
  if (end < customChannels.length) navigation.push({ text: "Next ➡️", callback_data: `custom_channels_${page + 1}` });
...
// Play a custom channel
async function playCustomChannel(chat_id, id) {
  let customChannels = customChannelsMap.get(chat_id) || [];
  if (id >= 0 && id < customChannels.length) {
    let channel = customChannels[id];
    let channelUrl = bypassGeo(channel.url);
