...
// In-memory detailed subscribers info: stores chat_id => {name, joinDate}
const subscribersInfo = new Map();
// In-memory unsubscribers info: stores chat_id => {name, joinDate, unsubscribeDate}
const unsubscribersInfo = new Map();
...
async function handleDashboard(request) {
  ...
  // Generate subscribers HTML
  let subscribersHTML = "";
  subscribersInfo.forEach((info, chat_id) => {
    subscribersHTML += `Chat ID: ${chat_id}, Name: ${info.name}, Joined: ${info.joinDate}<br>`;
  });
  
  // Generate unsubscribers HTML with join and unsubscribe dates
  let unsubscribersHTML = "";
  unsubscribersInfo.forEach((info, chat_id) => {
    unsubscribersHTML += `Chat ID: ${chat_id}, Name: ${info.name}, Joined: ${info.joinDate}, Unsubscribed: ${info.unsubscribeDate}<br>`;
  });
    ...
    <body>
      <div class="container">
        ...
        <div class="section">
          <h2>Subscribers</h2>
          <div class="info">
            Total Subscribers: ${subscribersInfo.size}<br>
            ${subscribersHTML}
          </div>
        </div>
        <div class="section">
          ...
          <div class="info">
            Total Unsubscribed: ${unsubscribersInfo.size}<br>
            ${unsubscribersHTML}
          </div>
        </div>
      </div>
  ...
  if (text === "/start") {
    let keyboard = {
      inline_keyboard: [
        ...
        // Removed the old share button. Added new share buttons with URL links.
        [{ text: "📤 Share to Chats", url: "https://t.me/Freeiptvstream_bot?start=share" }, { text: "📤 Share to Groups", url: "https://t.me/Freeiptvstream_bot?start=share" }],
...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    if (chat_id === ADMIN_CHAT_ID) {
      const statsMessage = `📊 Stats:\nTotal Subscribers: ${subscribersInfo.size}\nCached Channels: ${channelsCache.length}`;
      await sendMessage(chat_id, statsMessage);
    } else {
      await sendMessage(chat_id, "Access Denied: You are not authorized to perform this action.");
    }
  } else if (data === "subscribe") {
    // Add the user to subscribers and record detailed info
    subscribers.add(chat_id);
    subscribersInfo.set(chat_id, { name: query.from.first_name || "Anonymous", joinDate: new Date().toLocaleString() });
    await sendMessage(chat_id, "You have successfully subscribed to notifications.");
  } else if (data === "unsubscribe") {
    // Remove the user from subscribers and record unsubscribe info
    subscribers.delete(chat_id);
    if (subscribersInfo.has(chat_id)) {
      let info = subscribersInfo.get(chat_id);
      info.unsubscribeDate = new Date().toLocaleString();
      unsubscribersInfo.set(chat_id, info);
      subscribersInfo.delete(chat_id);
    } else {
      unsubscribersInfo.set(chat_id, { name: query.from.first_name || "Anonymous", joinDate: "unknown", unsubscribeDate: new Date().toLocaleString() });
    }
...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    // Store the custom channels for the user
    customChannelsMap.set(chat_id, channels);
    await sendMessage(chat_id, "Custom channels processed successfully and added.");
  } catch (error) {
    console.error("Error processing custom URL:", error);
    await sendMessage(chat_id, "Error processing custom URL.");
  }
}
...
async function processUploadedM3UFile(message) {
  ...
  // Retrieve the file info from Telegram
  const fileRes = await fetch(`${TELEGRAM_API}/getFile?file_id=${file_id}`);
  const fileData = await fileRes.json();
  if (!fileData.ok) {
    await sendMessage(chat_id, "Failed to retrieve file info.");
    return;
  }
  ...
  if (!response.ok) {
    await sendMessage(chat_id, "Failed to download file.");
    return;
  }
  const content = await response.text();
  console.log("Downloaded M3U file content:", content);
  ...
  customChannelsMap.set(chat_id, channels);
  await sendMessage(chat_id, "Uploaded M3U file processed successfully and channels added.");
}

// List custom channels with pagination for a user
async function listCustomChannels(chat_id, page, message_id = null) {
  ...
  let channelsPerPage = CHANNELS_PER_PAGE;
  let start = page * channelsPerPage;
...
// Play a custom channel for the user
