...
async function processTelegramMessage(message) {
  ...
  if (text === "/start") {
    // Automatically subscribe the user when they click /start
...
async function processCustomUrl(chat_id, customUrl) {
  try {
    ...
    if (channels.length === 0) {
      await sendMessage(chat_id, "No channels found in your custom file.");
      return;
    }
    
    // Save the custom channels for the user in customChannelsMap
...
// Process .m3u file uploaded by the user
async function processUploadedM3UFile(message) {
  const chat_id = message.chat.id;
  const file_id = message.document.file_id;
  // Get file path from Telegram API
  let filePathResponse = await fetch(`${TELEGRAM_API}/getFile?file_id=${file_id}`);
  let filePathData = await filePathResponse.json();
  if (!filePathData.ok) {
    await sendMessage(chat_id, "Failed to get file.");
    return;
  }
  let file_path = filePathData.result.file_path;
  // Construct file URL
  let file_url = `https://api.telegram.org/file/bot${BOT_TOKEN}/${file_path}`;
  try {
    let response = await fetch(file_url);
    if (!response.ok) {
      await sendMessage(chat_id, "Failed to download the file.");
      return;
    }
    ...
    // Save the custom channels under this user
    customChannelsMap.set(chat_id, channels);
    await sendMessage(chat_id, `Custom channels added successfully. Found ${channels.length} channels.`);
  } catch (error) {
...
async function listCustomChannels(chat_id, page, message_id = null) {
  ...
  if (!channels || channels.length === 0) {
    if (message_id) {
      await editMessage(chat_id, message_id, "No custom channels found. Please add a custom URL or upload a .m3u file.");
    } else {
