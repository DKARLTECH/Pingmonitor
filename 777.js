...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    // Prompt the user to send their custom URL (only m3u or m3u8_plus links are allowed)
    await sendMessage(chat_id, "Please send your custom URL (only m3u or m3u8_plus links are allowed):");
    await answerCallbackQuery(query.id, "Custom URL request acknowledged");
...
// Helper function to answer callback queries to remove loading state in Telegram client
async function answerCallbackQuery(callback_query_id, text = "") {
  try {
    await fetch(`${TELEGRAM_API}/answerCallbackQuery`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ callback_query_id: callback_query_id, text: text })
    });
  } catch (error) {
