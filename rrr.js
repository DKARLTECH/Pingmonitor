...
const ADMIN_CHAT_ID = 8049379557;
  ...
  if (text === "/start") {
    let keyboard = {
      inline_keyboard: [
        ...
        // Updated Add Custom URL button pop up message instruction via callback query handling
...
async function processCallbackQuery(query) {
  ...
  if (data.startsWith("channels_")) {
    ...
    // Set pending state and prompt the user to send their custom URL with updated instructions
    pendingCustomUrlMap.set(chat_id, true);
    await sendMessage(chat_id, "Please send your m3u/m3us_plus link to get Channels:");
